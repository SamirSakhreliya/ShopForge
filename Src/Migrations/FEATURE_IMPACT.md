...
| **Slack new-order alert**                                          | 7/10  | Vendors miss new orders without it. Silent order queue leads to SLA breaches and customer complaints. Non-blocking (fire-and-forget) so it doesn't impact order placement reliability. **Implemented (KAN-25)** via `OrderNotifier` on a dedicated `ORDER_SLACK_URL`/per-tenant `slack_orders_hook`, separate from the error-alert channel. |
| **Order cancellation flow**                                        | 7/10  | Customers expect cancellation within a window. Without it, support handles cancellations manually — high effort, error-prone. Stock must be returned on cancellation. **Partially implemented (KAN-25):** vendor can cancel (`PATCH .../status`) with automatic restock; no customer-initiated cancel endpoint yet.                         |
| **Multi-vendor cart checkout** (one cart, N vendor orders)         | 8/10  | Without it, a customer buying from two vendors needs two separate checkouts — friction that directly suppresses cross-vendor order volume. **Implemented (KAN-25):** `OrderService.checkout()` splits one cart into one order per tenant atomically.                                                                                        |
...
| Feature                                 | Score | Justification                                                                                                                                                                                                                                                                                                                                                     |
| --------------------------------------- | ----- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Per-tenant Slack orders hook**        | 7/10  | Enables each vendor to receive order alerts in their own Slack workspace. Without it, all alerts go to the platform, not the vendor — unscalable. **Now consumed (KAN-25)** by `OrderNotifier.resolveWebhookUrl()`, which checks this before falling back to the shared `ORDER_SLACK_URL`.                                                                        |
| **Vendor settings self-service API**    | 7/10  | Without a write path, the Slack hook above (and currency/theme/low-stock-threshold) are DB-edit-only — the exact gap that caused a silent failed Slack alert during KAN-25 testing (a seeded placeholder URL had no way to be fixed except by hand). **Implemented (KAN-25 follow-up)** via `GET/PUT /api/v1/vendor/settings`, allow-listed to these 4 keys only. |
